Change 07/21/26
* Changes in Compiler Spec. Actions to rebuild the different meta data (DTE/XLP/XODs/JSP/JAVA). And a button to rebuild all of the lplr meta data.

Change 07/15/26
* More Tomcat 11 change/fixes

Changes 07/09/26
* Added new code for Tomcat 11 and file upload. The upload code is now in the upload button action. Now uses request.getPart("wFileName"). Issues with uploading large files.

Changes 06/26/2026
* More changes for bootstrap5 grid text-end/start css in header.
...
Changes 02/23/26
* Added the Dialog Name to the Dialog Window top caption display. Now displays (Dialog.Window - Window Caption). Requested by Alan S

Changes 01/12/26
* Bitmap/BitmapBtn did not have HTML5 in the generation of the control (it did on bitmaps in grids). This has been added.

Changes 09/24/2025
* Have streamlined jsp generation code to try and get the jsp size limit down to avoid the 6555 limit crash.

Change 09/18/2025
* Had an issue where special characters in tzedfrmd.c were wiped out by notepad. Had to re-do my code changes using Notepad++

Change 09/18/2025
* In the VML editor, two new menu options "Operations->Dialog/Object Operations List" and "Operations->Global Operations List", which will bring up a list of operations that exist for
  either the Dialog, Object or Global Operations Group. If you double click on an operation in the list, the cursor will be positioned on that operation. One issue is that for a Dialog
  this is a list of all operations no matter the Source File. If an operation from a different source file is clicked, the cursor position is not changed.
  A separate list of only the operations in the current source would have required changes to TZWDLGSO.LOD which I did not wish to do because it can cause issues.
